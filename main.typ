...
The example in the @agsy_transformation_branches contains three possible kinds if refinements for the goal: 1) Replace it with local, 2) Replace it with function and create new subproblems for every argument, 3) Replace goal with a type constructor and create subgoals for all fields required by constructor.
...
#todo("end case seems to be missing")
...
#todo("functional pseudocode to show DFS")
...
#todo("say we propagate constrains only when discarding goal as solved")
