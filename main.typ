...
We'll be more interested in the proof assistants as they are the ones leveraging the term search.
As there are many alternatives we've picked some that seem the most popular or relavant for our use case.

#todo("Say which we picked and why in 1 sentence")
...
The tool uses problem collections and solution collections to keep track of the goals to prove and proofs for each goal.
...
Problem collection (`PrbColl`) is a collection of problems that all have to be solved for the proof to hold.
Solution collection (`SolColl`) is a collection of solutions for particular `PrbColl`.
Problem and solution collections are necessary for finding solutions that include case splitting.
This is because we have to show that we can find solution for every case introduced by case splitting.
...
Refinements are ways to transform the probkem generated by tactics.
...
The `searchColl` algorithm attempts to solve problem collection.
To do that we iterate over all the problems and attempt to solve them by calling `search` function.
If we do not find solution for a problem we return empty solution immideately as all the problems in the problem  collection need to be solved for the solution to hold.
...
However the authors of @tool-for-automated-theorem-proving-in-agda noted that wile the performace of the tool is good enough to be useful it performs poorly on larger problems.
They suggested that more advanced search space reduction techniques could be used as well as writing it in a language that does not suffer from automatic memory management.
It was also noted that there seem to be many false subproblems that can never be solved so they suggested parallel algorithm that could potentially prove the uselessness of those subproblems potentially faster to reduce the search space.
...
The rationale for this is that it is more likely that user wishes to use variables from local scope than from outside of it.
...
#todo("Apparently cannot find anything useful here. Most of the stuff seems to be how to validate program...")
...
It is also based on a DFS, but is optimized to maximally use memoization.
...
Smyth has many other opimizations but they focus on using the information from examples and are therefore not interesting for us. 
